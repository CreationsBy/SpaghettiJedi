...
// --- NEW FEATURE: Persisted Music Mute Logic ---
const muteBtn = document.getElementById('mute-btn');
// Load the muted state from localStorage, defaulting to false (not muted)
let isMuted = localStorage.getItem('fridgeMuted') === 'true';

// Set the initial audio state and UI box state on load
...
function updateMuteButtonUI() {
    if (isMuted) {
        ...
        muteBtn.style.backgroundColor = "#ff4d4d"; // Red when muted
    } else {
        muteBtn.innerText = "🔊 Music On";
        muteBtn.style.backgroundColor = "#555"; // Default gray when unmuted
    }
}

// Toggle the muted state when the user clicks the button
muteBtn.addEventListener('click', () => {
    isMuted = !isMuted;
    bgmPlayer.muted = isMuted;
    localStorage.setItem('fridgeMuted', isMuted); // Save the choice locally
    updateMuteButtonUI();
});
...
document.body.addEventListener('pointerdown', () => {
    if (!musicStarted && document.getElementById('instructions-modal').classList.contains('hidden')) {
        ...
        // Will play instantly, but will be silent if 'isMuted' is true
...
function renderItem(data) {
    ...
    if (data.type === 'magnet') {
        ...
        // --- UPDATED: Add span for brush size text next to the slider ---
        el.innerHTML = `
            ...
            <div class="drawing-toolbar">
                ...
                <span class="brush-size-text">Size: 3px</span> <button class="toggle-mode">Draw Mode</button>
            </div>
        ...
        const brushSizeText = el.querySelector('.brush-size-text'); // Added
        ...
        if (isOwner) {
            ...
            // --- NEW FEATURE: Update size text when slider moves ---
            brushSize.addEventListener('input', () => {
                brushSizeText.innerText = `Size: ${brushSize.value}px`;
            });
