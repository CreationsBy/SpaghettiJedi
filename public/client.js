...
function renderItem(data) {
    ...
    // --- UPDATED CLEAR BUTTON LOGIC FOR DESKTOP & MOBILE ---
    ...
    // 1. DESKTOP: Hover Logic (Ignores Touch/Pens)
    ...
    // 2. MOBILE: Tap & Hold Logic (Ignores Mouse)
    el.addEventListener('pointerdown', (e) => {
        if (e.pointerType === 'mouse') return; 
        
        // Prevent accidental triggers if they tap on active inputs
        if (e.target === clearBtn || e.target.tagName === 'INPUT' || e.target.tagName === 'BUTTON' || e.target.tagName === 'TEXTAREA') return;

        // Intuitive Mobile UI: Tap the note to dismiss the clear button if it's open
    ...
    // Cancel the long-press timer if they slide their finger more than 10 pixels
