...
// --- UNIVERSAL POINTER EVENTS FOR MINI-MAP ---
...
// Use pointerdown to start music universally on first interaction
...
function checkCollisionWithOthers(testLeft, testTop, width, height, myItemId) {
    for (let id in clientItems) {
        const other = clientItems[id];
        
        if (other.owner !== myId && other.id !== myItemId) {
...
function createNewItem(type, content) {
    ...
    let angle = 0;
    let radius = 0;
    while (checkCollisionWithOthers(spawnX, spawnY, myW, myH, itemId)) {
        radius += 15; 
        angle += Math.PI / 4; 
...
function renderItem(data) {
    ...
    if (data.type === 'magnet') {
        ...
        if (isOwner) {
            ...
            let isDrawing = false;
            // --- UNIVERSAL POINTER EVENTS FOR DRAWING ---
    ...
    el.addEventListener('mouseenter', resetIdleTimer);
...
// --- UNIVERSAL POINTER EVENTS FOR DRAGGING ---
function setupDragging(mainElement, dragTarget, data) {
    ...
    document.addEventListener('pointermove', (e) => {
        if (isDragging) {
            ...
            if (!checkCollisionWithOthers(targetX, newY, myW, myH, data.id)) newX = targetX;
            if (!checkCollisionWithOthers(newX, targetY, myW, myH, data.id)) newY = targetY;
