...
usernameInput.addEventListener('input', (e) => {
    // Only send the request to the server, do NOT overwrite the local name permanently yet!
...
socket.on('connect', () => {
    // --- NEW FIX: Ask the server for an official, unique name as soon as we connect! ---
    socket.emit('registerUser', { ownerId: myId, username: currentUsername });
});

// --- NEW FIX: Listen for the server to confirm what our actual name is ---
socket.on('usernameConfirmed', (confirmedName) => {
    currentUsername = confirmedName;
    usernameInput.value = currentUsername; // Update the UI box to show if a number was added
    localStorage.setItem('fridgeUsername', currentUsername); // Save it locally!
